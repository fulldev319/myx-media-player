...
import {apiAddTopicEmoji} from 'helper/slambookHelper';

const TopicAddEmojiPage = ({route, navigation}) => {
  const {slambookId, topicId, commentId} = route.params!;
  ...
  const onAddEmoji = async () => {
    ...
    const res = await apiAddTopicEmoji({
      slambook: slambookId,
      topic: topicId,
...
export default TopicAddEmojiPage;
